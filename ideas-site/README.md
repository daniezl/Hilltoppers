...
> **Paused**, along with the rest of the ideas board — the extension's Ideas
> section is off too (`IDEAS_ENABLED` in
> `chrome-extension/src/services/ideasService.ts`). The code stays so it can be
> picked back up once the shared palette and sign-in design are settled.
