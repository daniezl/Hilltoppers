...
export default function ToppingBar({ onAccount, active = true }: { onAccount?: () => void; active?: boolean }) {
  ...
  useEffect(()=>{
    ...
    if(!user){setCanReview(false);setPendingReviews(0);return;}
    const update=async()=>{
      if(!active || document.hidden || fetching)return;
      fetching=true;
      try {
        const data=await toppingRequest('/submissions');
        if(alive){setCanReview(data.canReview);setPendingReviews(data.canReview?data.toppings.filter((t:{status:string})=>t.status==='pending').length:0);}
      } catch {} finally {fetching=false;}
    };
    void update();
    const timer=window.setInterval(()=>void update(),60000);
    window.addEventListener('focus',update);document.addEventListener('visibilitychange',update);
    return()=>{alive=false;window.clearInterval(timer);window.removeEventListener('focus',update);document.removeEventListener('visibilitychange',update);};
  },[user?.uid,user?.emailVerified,active,reviewing,publishing,reviewRevision]);
  ...
  async function publish(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();if(imageEditing)return;const data=new FormData(event.currentTarget);
    await run(async()=>{
