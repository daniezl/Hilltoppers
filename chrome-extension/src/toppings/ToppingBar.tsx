...
export default function ToppingBar({ onAccount, active = true }: { onAccount?: () => void; active?: boolean }) {
  ...
  const linkedAuthor = linkedSchool?.split('@')[0].split(/[._-]+/).filter(Boolean).map(part=>part[0].toUpperCase()+part.slice(1)).join(' ');
  const signIn = () => {
  ...
  useEffect(() => { const close = (e: KeyboardEvent) => { if(active&&e.key==='Escape'&&!busy){setDetail(null);setPublishing(false);setCreating(false);setReadingGuide(false);} }; window.addEventListener('keydown',close);return()=>window.removeEventListener('keydown',close); },[busy,active]);
  useEffect(() => {
    if (!active || (!creating && !publishing && !detail && !readingGuide)) return;
  ...
  }, [active, creating, publishing, detail?.id, readingGuide]);
  ...
  async function publish(event: React.FormEvent<HTMLFormElement>) {
    ...
    await run(async()=>{
      ...
      const author = linkedAuthor || String(data.get('author') || '').trim();
      if (author.length < 2 || /^anonymous$/i.test(author)) throw new Error('Enter your real name.');
      if (!linkedAuthor && author !== user.displayName) { await updateProfile(user, { displayName: author }); await user.getIdToken(true); }
  ...
  return <div className="bar-page">
    ...
    {readingGuide&&<div className="modal-backdrop" onClick={event=>{if(event.target===event.currentTarget)setReadingGuide(false);}}><section className="modal topping-guide" role="dialog" aria-modal="true" aria-labelledby="topping-guide-title"><button className="close" aria-label="Close" onClick={()=>setReadingGuide(false)}>×</button><ToppingGuide/></section></div>}
    {creating&&<div className="modal-backdrop" onClick={event=>{if(event.target===event.currentTarget&&!busy)setCreating(false);}}><section className="modal creator-modal" role="dialog" aria-modal="true" aria-labelledby="creator-title"><button className="close" aria-label="Close" disabled={busy} onClick={()=>setCreating(false)}>×</button><div className="eyebrow">TRY IT IN HILLTOPPERS</div><h2 id="creator-title">Preview your Topping.</h2><p>Enter a hosted URL or localhost address to try your Topping in the extension.</p><form onSubmit={preview}><label>Topping name<input name="name" maxLength={48} defaultValue="Preview Topping"/></label><label>Topping URL<input name="url" type="url" required placeholder="https://your-topping.example"/></label><p className="form-note">The webpage must allow embedding in an iframe.</p>{error&&<p role="alert" className="error">{error}</p>}<button className="primary" disabled={busy}>{busy?'Adding…':'Add preview'}</button>{previewAdded&&<p className="preview-success" role="status">Preview added. Open the extension to see it.</p>}</form><div className="creator-publish"><div><strong>Ready to share it?</strong><p>Publish your finished Topping to the community.</p></div><button className="quiet creator-publish-button" onClick={()=>{setCreating(false);setPublishing(true);}}>Publish a Topping</button></div></section></div>}
    {publishing&&<div className="modal-backdrop" onClick={event=>{if(event.target===event.currentTarget&&!busy)setPublishing(false);}}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="publish-title"><button className="close" aria-label="Close" disabled={busy} onClick={()=>setPublishing(false)}>×</button><div className="eyebrow">MADE BY YOU</div><h2 id="publish-title">Share your Topping.</h2>{!schoolAccount?<><p>Link a verified school email to publish. Your name will be public; your email stays private.</p><button className="primary" onClick={signIn}>{user ? 'Link school email' : 'Sign in'}</button></>:<form onSubmit={publish}><p className="form-note">Publishing as <strong>{linkedAuthor || user?.displayName||'your account name'}</strong>.</p>{!linkedAuthor && <label>Author name<input name="author" required minLength={2} maxLength={80} defaultValue={user?.displayName || ''}/></label>}<fieldset className="topping-icon-picker"><legend>Icon</legend><div>{TOPPING_ICONS.map(({id,label})=><button key={id} type="button" aria-label={label} aria-pressed={selectedIcon===id} title={label} onClick={()=>setSelectedIcon(id)}><ToppingIcon icon={id}/></button>)}</div></fieldset><label>Name<input name="name" required minLength={2} maxLength={48}/></label><label>Short description<textarea name="description" required minLength={10} maxLength={180} rows={2}/></label><label>Topping URL<input name="url" type="url" required placeholder="https://…"/></label><label>Preview image URL<input name="image" type="url" required placeholder="https://…/preview.png"/></label><p className="form-note">Use a landscape image. Your Topping must allow embedding in an iframe.</p>{error&&<p role="alert" className="error">{error}</p>}<button className="primary" disabled={busy}>{busy?'Publishing…':'Publish Topping'}</button></form>}</section></div>}
