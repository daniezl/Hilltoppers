...
export async function getAllCachedSpecialDays(): Promise<Record<string, CachedSpecialDay> | null> {
  const cached = await getCacheItem<CachedSpecialDays>(CACHE_KEYS.SPECIAL_DAYS);
  if (!cached || !isCacheValid(cached, CACHE_DURATION_MS)) {
    return null;
  }
...
export async function removeCachedSpecialDay(dateKey: string): Promise<void> {
  const cached = await getCacheItem<CachedSpecialDays>(CACHE_KEYS.SPECIAL_DAYS);
  if (!cached || !cached.days[dateKey]) {
    return;
  }
  
  delete cached.days[dateKey];
